...
# Dynamic Factor Model (dfm_nowcast) Path Integration
DFM_LOCAL_PATH = os.path.abspath(os.path.dirname(__file__))
DFM_PARENT_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "dfm_nowcast"))
for p in [DFM_LOCAL_PATH, DFM_PARENT_PATH]:
...
try:
    ...
    HAS_DFM_NOWCAST = True
except Exception as e_dfm:
    HAS_DFM_NOWCAST = False

# Abaikan warning agar terminal bersih
warnings.filterwarnings('ignore')

DATA_DIR = os.environ.get("DATA_DIR", os.path.dirname(os.path.abspath(__file__)) if '__file__' in globals() else os.getcwd())
file_makro = os.path.join(DATA_DIR, "Makro Indikator AI.xlsx")
file_adb = os.path.join(DATA_DIR, "INO_130726.xlsx")
...
@st.cache_data
def load_data(file_makro_path=file_makro, file_adb_path=file_adb):
    try:
        df_target = pd.read_excel(file_makro_path, sheet_name=0)
        df_triwulan = pd.read_excel(file_makro_path, sheet_name=1)
        df_makro = pd.read_excel(file_makro_path, sheet_name=2)
        df_hist_gdp = pd.read_excel(file_adb_path, sheet_name=2)
        return df_target, df_triwulan, df_makro, df_hist_gdp
    except Exception as e:
        st.error(f"⚠️ Gagal memuat data Excel ({file_makro_path} / {file_adb_path}): {e}")
...
@st.cache_data(show_spinner="⚙️ DFM Engine (dfm_nowcast): Menghasilkan Histori Prediksi & Evaluasi Risiko...")
def run_full_dfm_replication(data_file_path=file_adb, threshold=5.0, threshold_direction="lower", native=True):
    try:
        out_dir = os.path.join(DATA_DIR, "output_dfm")
        ...
        if HAS_DFM_NOWCAST:
            ...
            res_files = sorted([f for f in os.listdir(out_dir) if f.startswith('Result_') and f.endswith('.xlsx')])
            if res_files:
                res_path = os.path.join(out_dir, res_files[-1])
                df_pred = pd.read_excel(res_path, sheet_name='Prediction')
                if not df_pred.empty:
                    prob_now_col = [c for c in df_pred.columns if 'Prob Nowcast' in c]
                    prob_ann_col = [c for c in df_pred.columns if 'Prob Annual' in c]
                    df_pred['Prob_Nowcast'] = df_pred[prob_now_col[0]] / 100.0 if prob_now_col and df_pred[prob_now_col[0]].max() > 1 else (df_pred[prob_now_col[0]] if prob_now_col else np.nan)
                    df_pred['Prob_Annual'] = df_pred[prob_ann_col[0]] / 100.0 if prob_ann_col and df_pred[prob_ann_col[0]].max() > 1 else (df_pred[prob_ann_col[0]] if prob_ann_col else np.nan)
                    df_pred['Prob_Seq'] = np.nan
                    return df_pred

        # Fallback statsmodels replication if dfm_nowcast fails or is unavailable
...
if main_menu == "📊 Makro Nasional (DFM)":
    ...
    if df_target is not None:
        ...
        if not df_full_results.empty:
            ...
            if isinstance(ref_q_str, (pd.Timestamp, datetime)):
                ref_year = ref_q_str.year
                ref_q_num = (ref_q_str.month - 1) // 3 + 1
            else:
                ref_q_str = str(ref_q_str)
                ref_year = int(ref_q_str[:4])
                ref_q_num = int(ref_q_str[-1]) if ref_q_str[-1].isdigit() else 1
        ...
        # --- PROBABILITY RISK CARDS (dfm_nowcast Engine) ---
        if not df_full_results.empty and 'Prob_Nowcast' in df_full_results.columns:
            latest_prob_row = df_full_results.sort_values('Day Prediction').iloc[-1]
            p_now_val = latest_prob_row.get('Prob_Nowcast', np.nan)
            p_ann_val = latest_prob_row.get('Prob_Annual', np.nan)
            p_seq_val = latest_prob_row.get('Prob_Seq', np.nan)
            
            st.session_state['dfm_prob_nowcast'] = p_now_val
            st.session_state['dfm_prob_annual'] = p_ann_val
            st.session_state['dfm_prob_seq'] = p_seq_val
            
            st.markdown('<div class="glass-card">', unsafe_allow_html=True)
            st.markdown("### 🛡️ Evaluasi Risiko & Probabilitas Breaching (`dfm_nowcast`)")
            pr1, pr2, pr3 = st.columns(3)
            with pr1:
                p_now_str = f"{p_now_val * 100:.1f}%" if pd.notna(p_now_val) else "N/A"
                st.metric("Risk Target Breaching P(Nowcast < 5.0%)", p_now_str)
                st.caption("Probabilitas GDP kuartalan di bawah threshold 5.0%")
            with pr2:
                p_ann_str = f"{p_ann_val * 100:.1f}%" if pd.notna(p_ann_val) else "N/A"
                st.metric("Risk Annual Breaching P(Annual GDP < 5.0%)", p_ann_str)
                st.caption("Probabilitas GDP tahunan di bawah 5.0%")
            with pr3:
                p_seq_str = f"{p_seq_val * 100:.1f}%" if pd.notna(p_seq_val) else "N/A"
                st.metric("Recession Risk P(Sequential Breach)", p_seq_str)
                st.caption("Risiko resesi teknikal (2 kuartal berturut-turut < target)")
            st.markdown('</div>', unsafe_allow_html=True)
        
        if not df_full_results.empty:
            st.markdown('<div class="glass-card">', unsafe_allow_html=True)
            st.markdown("#### 📥 Ekspor Laporan DFM Nowcasting")
            ...
            with d_col1:
                ...
                st.download_button("📊 Unduh Laporan Excel (Full DFM Results)", data=buffer.getvalue(), file_name="Nowcasting_DFM_Report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            with d_col2:
                # Generate PowerPoint deck if template exporter is available
                out_dfm_dir = os.path.join(DATA_DIR, "output_dfm")
                pptx_files = [os.path.join(out_dfm_dir, f) for f in os.listdir(out_dfm_dir) if f.endswith('.pptx')] if os.path.exists(out_dfm_dir) else []
                if pptx_files:
                    with open(pptx_files[0], "rb") as f_pptx:
                        st.download_button("📙 Unduh Deck Presentasi PowerPoint (.pptx)", data=f_pptx.read(), file_name="DFM_Nowcasting_Presentation.pptx", mime="application/vnd.openxmlformats-officedocument.presentationml.presentation", use_container_width=True)
                else:
                    st.info("Presentasi PPTX dapat didownload setelah ekspor otomatis.")
            st.markdown('</div>', unsafe_allow_html=True)
